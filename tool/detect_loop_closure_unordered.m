...
%
% INPUTS
%   edges     : struct array with fields .type (char in {'P','L','G'}), .from, .to
%   state     : struct with running state (may be empty on first call)
...
if nargin < 2 || isempty(state)
  state.pose_order = containers.Map('KeyType','double','ValueType','double');
  state.lm2orders  = containers.Map('KeyType','double','ValueType','any');
  state.next_order = 1;
else
  % Ensure required fields exist
  if ~isfield(state,'pose_order') || ~isa(state.pose_order,'containers.Map')
    state.pose_order = containers.Map('KeyType','double','ValueType','double');
  end
  if ~isfield(state,'lm2orders') || ~isa(state.lm2orders,'containers.Map')
    state.lm2orders = containers.Map('KeyType','double','ValueType','any');
  end
  if ~isfield(state,'next_order') || ~isscalar(state.next_order)
    state.next_order = 1;
  end
end
